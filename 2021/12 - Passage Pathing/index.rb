...
def find_paths_helper(from, to, graph, allowed_duplicate_node, visited_nodes = [], valid_paths = [])
    return valid_paths << visited_nodes + [to] if from == to
    
    graph[from].each do |node|
      if visited_nodes.include?(node) && node.downcase == node
        next unless node == allowed_duplicate_node && visited_nodes.count(node) == 1
      end
  
      find_paths_helper(node, to, graph, allowed_duplicate_node, visited_nodes + [from], valid_paths)
    end  
...
def solve_part_two(graph)
    valid_paths = []
    allowed_duplicate_nodes = graph.keys.select {
        |node| node.downcase == node && [:start, :end].include?(node) == false
    }
